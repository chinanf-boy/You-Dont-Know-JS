...
更重要的是，如果你通过一个非承诺thenable价值`Promise.resolve(..)`，它会尝试打开那个价值，并展开将继续直到最后非诺言的价值是具体的提取。
...
尽管如此，我们可以通过这些版本中的任何一个`P`到`Promise.resolve(..)`我们会得到正常、安全的结果：
...
`承诺，决心（…）`将接受任何thenable，将打开它的非thenable价值。但你可以从`承诺，决心（…）`一个真正的、真正的承诺，**一个你可以信任的人**。如果你所传递的已经是一个真正的承诺，你就把它立刻收回，这样就没有任何负面影响了。`Promise.resolve(..)`获得信任。
...
我们首先定义一个`请求（…）`构造一个承诺来代表完成的实用程序。`ajax（..）`呼叫：
...
使用承诺返回`请求（…）`我们通过第一个URL调用它来隐式地创建链中的第一步，并将第一个返回的承诺链起来。`然后（…）`。

一旦`response1`返回时，我们使用该值来构造第二个URL，然后生成第二个URL。`请求（…）`呼叫。第二`请求（…）`承诺是`返回`ED让我们异步流控制第三步等待Ajax调用完成。最后，我们打印`反应`一旦它回来。
...
`Promise.resolve(..)`创造一个被赋予价值的承诺。在这个例子中，`四十二`是正常的，不答应，非thenable价值，所以履行承诺`fulfilledpr`是为价值而创造的`四十二`。`承诺。拒绝（“哎呀”）`创造被拒绝的承诺`rejectedpr`的原因`“哎呀”`。

现在让我们解释一下“决心”这个词的原因（例如`Promise.resolve(..)`）如果在上下文中明确地使用，可以实现或拒绝，这是明确且确实更准确的：
...
正如我们在本章前面所讨论的那样，`Promise.resolve(..)`将接收到的真正的承诺，或打开收到的thenable。如果thenable展开了拒绝状态，答应回来`承诺，决心（…）`其实是在同一个拒绝状态。

所以`Promise.resolve(..)`是API方法的一个好的、准确的名称，因为它实际上可以实现或拒绝。
...
**注：**以下仅为6 API是土生土长的，但也有符合规范的polyfills（不只是延长保证图书馆）可以定义`承诺`和所有相关的行为，你甚至可以在pre-es6浏览器使用本地的承诺。其中一个是“本地承诺只polyfill”（<http://github.com/getify/native-promise-only>), which I wrote!
...
承诺，拒绝（…）`所以这两个承诺是等价的：`Promise.resolve(..)
...
`通常是用类似的方法来创造一个已经实现的承诺。`承诺，拒绝（…）`。然而,`Promise.resolve(..)`同时打开thenable值（讨论好几次了）。在这种情况下，采用的thenable承诺返回你通过的最后决议，这可能是履行或拒绝：`记住，
...
所以，`承诺（ajax）`产生一个`ajax（..）`promisory我们称`请求（…）`，这promisory生成Ajax响应承诺。
