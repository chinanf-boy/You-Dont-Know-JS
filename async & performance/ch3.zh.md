...
然而,在我们介绍如何以这种方式工作的承诺,我们将获得的代码,我们已经明白,回调!ℴℴ如何_未来价值_ㄢ
...
当然,这种粗糙的回调方法不尽人意ㄢ这只是实现推理的好处的第一小步ㄢ_未来价值_不必担心何时可用或不可用ㄢ
...
如果出了什么问题`X`或`Y`或者,在添加过程中,不知何故失败了ㄢ`添加(..)`返回被拒绝,第二个回调错误处理程序传递给`然后(ⅆ)`将从承诺中得到拒绝值ㄢ
...
首先,没有什么(甚至不是JS错误)可以阻止承诺通知它的解决方案(如果它已经解决)ㄢ如果你登记一个许诺履行和拒绝回调,并承诺会解决,其中一个回调函数将总是被称为ㄢ
...
如果要传递多个值,则必须将它们包装在传递的另一个值中,例如`阵列`或一个`对象`ㄢ
...
除了原则上的违反,这种行为可能会造成破坏,如果说有多重的话ㄢ`然后(ⅆ)`注册回调的承诺`P`因为有些人会被打电话,而另一些人不会打电话,而且为什么会很不透明ㄢ
...
**注: **包装的另一个有益的副作用`承诺,决心(ⅆ)`在任何一个函数的返回值(thenable或不)是它的规范,函数调用到行为的异步任务的一种简单方法ㄢ如果`美孚(42)`有时返回即时值,或返回其他时间的承诺,`承诺(解决方案(富(42)))`确保它总是一个承诺的结果ㄢ避免Zalgo是更好的代码ㄢ
...
所以现在第一个`然后(ⅆ)`在一个异步序列的第一步,第二`然后(ⅆ)`是第二步ㄢ只要你需要它,它就会一直持续下去ㄢ继续链接以前的链接`然后(ⅆ)`每个自动创建的承诺ㄢ
...
你可以看到,假设排斥处理器只是重新抛出错误,最终迫使`P2`(链式承诺)以同样的错误原因拒绝ㄢ本质上,这允许错误沿着承诺链继续传播,直到遇到一个显式定义的拒绝处理程序ㄢ
...
在第一个参数的情况下`然后(ⅆ)`毫无疑问,它始终是满足的情况,因此不需要"解决ℽ术语的对偶性ㄢ作为一个方面说明,6规范使用`onfulfilled(..)`和`onrejected(..)`这些标签两个回调,因此准确的术语ㄢ
...
这种错误处理在技术上是正确的ㄢ_异步可_但它一点也写不好ㄢ错误的第一个回调交织在一起的这些无处不在的多层次`如果`语句检查不可避免地会导致回调地狱的危险(参见第2章)ㄢ
...
因为我们没有把拒绝处理程序传递给`然后(ⅆ)`默认的处理程序被替换,它只将错误传播到链中的下一个承诺ㄢ因此,两个错误都会出现ㄢ`P`和错误来了_之后_ `P`在其决议中(如`tolowercase()味精ㄢ`一个)过滤到最后ㄢ`handleerrors(..)`ㄢ
...
如果承诺被拒绝,则默认为向开发人员控制台报告该事实(而不是默认为静默)ㄢ您可以隐式地退出该报告(通过拒绝之前注册错误处理程序),或者显式地(使用`defer()`)ㄢ无论哪种情况,_你_控制假阳性ㄢ
...
但如果任务基本上都是异步的,或可以/应该执行的同时,您可以使用这些工具的异步版本的许多图书馆提供ㄢ
...
让我们回顾6`承诺`我们已经看到的API在本章中以零碎的形式展开ㄢ
...
每个承诺实例(**不**这个`承诺`api命名空间)`然后(ⅆ)`和`抓住(ⅆ)`方法,允许注册实现和拒绝处理程序的承诺ㄢ一旦承诺得到解决,将调用一个或另一个这些处理程序,但不是同时调用,并且将异步调用它(参见第1章中的"作业ℽ)ㄢ
...
**警告: **小心!如果一个空`阵列`通过对`承诺(全部)])`它会立即完成,但是`承诺(赛跑)])`将永远挂起永不解决ㄢ

6`承诺`API是非常简单和直接的ㄢ服务最基本的异步情况下它至少足够好,是一种开始时,重新安排你的代码从回调地狱更好的东西的好地方ㄢ

但有很多复杂的应用往往需要异步承诺自己将被限制在解决ㄢ在下一节中,我们将深入了解这些局限性,作为希望库获益的动机ㄢ

## 承诺的局限性

本节中我们将讨论的许多细节已经在本章中提到过,但我们只需具体回顾这些局限性ㄢ

### 序列的错误处理

我们在本章前面详细介绍了承诺错误处理ㄢ承诺设计的局限性ℴℴ它们如何连锁,特别是创建一个非常容易的陷阱,其中一个承诺链中的错误可以被悄悄地忽略ㄢ

但还有一些其他的事情要考虑承诺错误ㄢ因为承诺链不过是连接在一起的组织承诺,没有实体将整个链称为单个链ㄢ_事情_这意味着没有外部的方法来观察可能发生的任何错误ㄢ

如果构造一个没有错误处理的承诺链,那么链中的任何错误都会无限地传播到链中,直到被观察到(通过在某个步骤注册拒绝处理程序)ㄢ所以,在那个特定的情况下,引用_最后的_链条上的承诺就足够了ㄢ`P`在下面的代码段中,因为您可以在那里注册拒绝处理程序,它将被通知任何已传播的错误: 
...
虽然它可能似乎微妙的困惑,`P`这里没有指向链中的第一个承诺ㄢ`美孚(42)`打电话来,而不是从最后一个承诺,一个来自`然后(第)`呼叫ㄢ

同时,在保证链没有步显著做自己的错误处理ㄢ这意味着您可以注册一个拒绝错误处理程序ㄢ`P`如果链中的任何地方出现错误,都会得到通知: 

    p.catch( handleErrors );

但是,如果链中的任何一个步骤确实执行了自己的错误处理(也许隐藏了/从您所能看到的抽象),您的`handleerrors(..)`不会被通知ㄢ这可能是你想要的,毕竟,这是一个"处理拒绝ℽ,但它也可能ㄢ_不_做你想做的ㄢ完全缺乏通知(已经处理过的)拒绝错误的能力是限制某些用例中能力的限制ㄢ

它基本上与一个`试试ㄢ赶上`它可以捕获异常并简单地吞下它ㄢ所以这不是一个限制**唯一的承诺**,但它_是_我们希望有一个解决方法ㄢ

不幸的是,很多时候都没有保证承诺链序列中的中间步骤,所以如果没有这样的引用,就不能附加错误处理程序来可靠地观察错误ㄢ

### 单值

定义上的承诺只有一个实现价值或一个拒绝理由ㄢ在简单的例子中,这不是T

典型的建议是构造一个值包装器(例如`对象`或`阵列`)包含这些多个消息ㄢ这种解决方案,但它可以是相当尴尬和包,打开你的信息与你的诺言链每一环节繁琐ㄢ

#### 分裂的价值观

有时你可以把这个作为一个信号,你可以/应该把问题分解成两个或更多的承诺ㄢ
...
首先,让我们重新安排什么`富(ⅆ)`退货,这样我们就不用包装了ㄢ`X`和`Y`为一个单一的`阵列`运输价值ㄢ相反,我们可以将每个值包装到自己的承诺中: 
...
是一个`阵列`承诺真的比一个更好`阵列`通过一个承诺的价值观?在语法上,这是不是一个很大的改进ㄢ

但这种方法更接近承诺设计理论ㄢ它现在更容易在未来重构分裂计算`X`和`Y`分成不同的函数ㄢ它更清洁,更灵活,让呼叫代码决定如何编排这两个承诺ℴℴ使用`承诺(全部)])`在这里,但肯定不是唯一的选择ℴℴ而不是把这些细节抽象出来ㄢ`富(ⅆ)`ㄢ

#### 打开/展开争论

这个`变量=ⅆ`和`变量为ⅆ`任务仍然是笨拙的开销ㄢ我们可以使用一些功能的诡计(感谢Reginald Braithwaite,@ raganwald在推特)在辅助工具: 
...
这些技巧可能是整洁的,但6为我们更好的回答: 解构ㄢ该阵列将分配形式看起来像这样: 
...
**注: **在6解构形式的更多信息,参见_6与超越_本系列的标题ㄢ

### 单分辨率

承诺的最本质的行为之一是,诺言只有一次解决(履行或拒绝)ㄢ许多异步用例,你只检索一个值一次,所以这工作很好ㄢ

但也有很多异步例融入一个不同的模式,更类似于事件和/或数据流ㄢ表面上还不清楚承诺能在这种用例中应用得多好,如果有的话ㄢ如果没有在承诺之上的重要抽象,它们将完全无法处理多个值解析ㄢ

想象这样一个场景,你可能要发生一系列刺激措施(如异步事件),事实上可以发生多次,如单击按钮ㄢ
...
如果您的应用程序只需要单击一次按钮,那么这里的行为只起作用ㄢ如果第二次单击按钮,则`P`诺言已经解决,所以第二个`决心(ⅆ)`呼叫将被忽略ㄢ
...
这种方法将_工作_在这一个全新的承诺序列将被解雇为每一个`"点击ℽ`按钮上的事件ㄢ

但除了在事件处理程序内部定义整个承诺链的丑陋之外,这种设计在某些方面违反了关注/功能分离(SOC)的思想ㄢ您很可能希望在代码中的不同位置定义事件处理程序,并在其中定义_响应_对事件(承诺链)ㄢ在这种模式下,没有辅助机制,这是相当尴尬的ㄢ

**注: **阐明这种限制的另一种方法是,如果我们能够构造某种"观察到的ℽ,我们就可以订阅一个承诺链ㄢ有了这些抽象概念(如rxjs --图书馆<http://rxjs.codeplex.com/>但是,抽象的东西看起来太重了,你甚至看不到承诺的本质了ㄢ如此沉重的抽象带来了重要的问题,如是否(无承诺)这些机制是_可信赖的_正如承诺本身被设计成ㄢ我们将回顾附录B中的"可观察ℽ模式ㄢ

### 惯性

在自己的代码中开始使用承诺的一个具体障碍是当前存在的所有代码,这些代码还没有实现承诺ㄢ如果您有许多基于回调的代码,那么只需保持相同样式的编码就容易得多ㄢ

"运动中的代码库(用回调函数)将保持运动(有回调)除非是按一个聪明,知道开发商的承诺ㄢℽ

承诺提供了一种不同的范式,因此,代码的方法可以从一点点不同,在某些情况下,根本不同ㄢ你必须对它有意,因为承诺不会自然而然地从同样的"做代码ℽ的方式中摆脱出来ㄢ
...
正如我们前面提到过的,我们确实需要一个Ajax应用程序,它是基于承诺感知的,而不是基于回调的,我们可以调用它ㄢ`请求(ⅆ)`ㄢ你可以自己做,就像我们已经做的一样ㄢ但有手动定义承诺意识到包装每回调基于效用的开销使得它不太可能你会选择重构承诺意识的编码在所有ㄢ

承诺没有对这个限制作出直接的回答ㄢ然而,大多数承诺库确实提供了一个助手ㄢ但即使没有图书馆,想象这样的帮手: 
...
好吧,这不仅仅是一个极小的实用程序ㄢ然而,虽然看起来有点吓人,但并不像你想象的那么糟糕ㄢ它需要一个函数,它期望一个错误的第一样式回调作为它的最后一个参数,并返回一个新的,它自动创建一个返回的承诺,并为您替换回调,连接到承诺实现/拒绝ㄢ
...
`承诺,包装(ⅆ)`做**不**作出承诺ㄢ它产生一个能产生承诺的功能ㄢ从某种意义上说,一个承诺生产函数可以被视为一个"厂承诺ㄢ"我提出"promisoryℽ为名称这样的东西("承诺ℽ+"工厂ℽ)ㄢ

将一个回调等功能是一个承诺感知功能的行为有时也被称为"提升ℽ或"promisifyingℽㄢ但似乎没有一个标准的术语,什么叫合力作用比其他"解禁ℽ功能,所以我喜欢"promisoryℽ更好,我认为这是更具描述性的ㄢ

**注: **promisory不是由项ㄢ这是一个真正的词,它的定义意味着包含或传递诺言ㄢ这正是这些函数所做的,因此它是一个非常完美的术语匹配!

所以,`承诺(ajax)`产生一个`ajax(..)`promisory我们称`请求(ⅆ)`,这promisory生成Ajax响应承诺ㄢ

如果所有的功能都已经promisories,我们不需要自己制造的,所以额外的步骤是一个耻辱的点ㄢ但至少包装图案是(通常)可重复的,所以我们可以把它放进一个`承诺,包装(ⅆ)`帮助显示我们的承诺编码ㄢ
...
当然,我们正在进行重构`富(ⅆ)`to use our new`请求(ⅆ)`promisory,我们可以使`富(ⅆ)`一个promisory本身,而不是保持基于回调的需要制作和使用后续`betterfoo(..)`promisoryㄢ这个决定取决于是否`富(ⅆ)`需要保持回调为基础,与代码库的其他部分兼容ㄢ
...
而6承诺不来船等promisory包装助手,大多数图书馆提供,或者你可以做你自己ㄢ无论哪种方式,这种特殊的承诺限制是没有太多痛苦(当然比地狱的痛苦!)ㄢ

### 承诺uncancelable

一旦你创建了一个承诺,并为它注册了一个实现和/或拒绝处理程序,如果发生了其他的事情使这个任务变得毫无意义,那么没有什么可以阻止这个进程ㄢ

**注: **许多承诺抽象库提供了取消承诺的便利,但这是一个可怕的想法!许多开发商希望许诺本身是由外部的消除能力,但问题是,它会让一个承诺一个消费者/观察者影响其他一些消费者的观察能力同样的承诺ㄢ这违反了未来价值的信任(外部性),而且是"在距离"反模式作用的体现([http://en.wikipedia.org/wiki/action_at_a_distance_28computer_programming % % 29](http://en.wikipedia.org/wiki/Action_at_a_distance_%28computer_programming%29))ㄢ无论多么有用看来,它会引导你回到相同的噩梦为回调ㄢ
...
"超时ℽ是承诺的外在内容ㄢ`P`,所以`P`自己一直在走,我们可能不想要ㄢ
...
这是丑陋的ㄢ它有效,但远非理想ㄢ一般来说,你应该尽量避免这种情况ㄢ

但是如果你不能,这个解决方案的丑陋应该是一个线索ㄢ_取消_是一个功能,属于更高层次的抽象之上的承诺ㄢ我建议您希望向抽象库提供帮助,而不是自己动手ㄢ

**注: **我的_asynquence_承诺抽象库提供了这样一个抽象和`abort()`序列的能力,所有这些将在附录A中讨论ㄢ

单一的承诺并不是真正的流量控制机制(至少不是非常有意义的),这正是什么?_取消_指的是;这就是为什么承诺取消会觉得很尴尬ㄢ

相比之下,一连串的承诺被集体地结合在一起ℴℴ我喜欢称之为"序列ℽℴℴ_是_流控制表达式和

没有个人的承诺应该是可撤销的,但这一明智的_序列_要取消,因为你不传一个序列作为一个单一的不变的价值就像你做一个承诺ㄢ

### 保证性能

这种特殊的限制既简单又复杂ㄢ

比较多少件采用异步任务链与承诺链基本回调运动,它是明确的承诺有一个公平一点的事情,这意味着他们天生就至少慢了一点ㄢ回想起刚刚的信任保证承诺提供简单的列表,而特设的解决方案代码你要的层上的回调来达到同样的保护ㄢ

更多的工作要做,更多的警卫来保护,意味着承诺ㄢ_是_慢相比赤裸裸的ㄡ不可信任的回调ㄢ这是显而易见的,而且很简单的包装你的大脑ㄢ

但是要慢多少呢?好...事实证明,这是一个绝对难以回答的问题ㄢ

坦白地说,这是一种苹果对橙子的比较,所以这可能是一个错误的问题ㄢ实际上,您应该比较一个手动分层的自动回调系统是否比承诺实现更快ㄢ

如果承诺有一个合法的性能限制,它更多的是,他们真的不提供一项选择,信任保护你想要/需要与否,你让他们,永远ㄢ

然而,如果我们承认诺言一般是_稍微慢_比其非承诺,不可信的回调当量--假设有地方你觉得你可以证明信任的缺乏,这就意味着承诺应避免一刀切,好像你的整个应用程序是由什么但必须绝对最快的代码可能吗?

理智检查: 如果您的代码是合法的,**JavaScript是这样的任务的正确语言吗?**JavaScript可以优化运行应用程序非常performantly(见5章和6章)ㄢ但过于关注承诺小性能的权衡,在所有的好处他们提供光,_真正地_合适吗?

另一个微妙的问题是承诺ㄢ_一切_异步,这意味着一些立即(同步)完成步骤还是推迟到工作的下一步进展(见1章)ㄢ这意味着,它可能是一个序列的任务可以完成承诺永远比同序列接上回调稍慢ㄢ

当然,这里的问题是: 这些潜在的失误在性能的细微部分吗?_值得的_我们在这一章所阐述的承诺的所有其他利益?

我的看法是,在几乎所有的情况下,你可能会认为保证性能足够慢以而言,它实际上是一个反模式优化掉承诺的可信性和可组合性的好处,避免它们ㄢ

相反,您应该默认在代码库中使用它们,然后配置和分析应用程序的热(关键)路径ㄢ是承诺_真正地_是一个瓶颈,还是仅仅是理论上的减速?只有_然后_有实际有效的基准(见第6章),有责任和谨慎地在刚刚确定的关键领域内作出承诺吗?ㄢ

承诺慢一点,但是作为交换你得到很多信任,不该预测,和组合性建ㄢ也许限制实际上不是他们的表现,而是你对他们利益的缺乏感知?

## 回顾

承诺是可怕的ㄢ使用它们ㄢ他们解决_控制反转_问题困扰着我们的回调代码ㄢ

他们没有摆脱回调,他们只是把这些回调的编排一个值得信赖的中介机构,在我们和另一个效用之间ㄢ

保证连锁店也开始地址(当然不是绝对)以连续的方式更好的表达方式异步流,这有助于我们的大脑计划保持异步js代码更好ㄢ我们将看到一个更好的解决方案ㄢ_那个_下一章的问题!
