...
**注：**您可能会看到大多数其他js文档/代码，它们将格式化生成器声明为`功能* foo() { ..}`而不是我在这里做的`功能* foo() { ..}`-唯一的区别是风格的定位`*`。这两种形式的功能/语法相同，为第三`功能* foo() { ..}`（无空格）窗体。这两种风格都有争论，但我基本上更喜欢。`函数*…`因为当我用一个生成器引用一个生成器时，它就匹配了。`* foo()`。如果我只说`foo()`如果我说的是发电机或正则函数，你不会很清楚。纯粹是文体上的偏爱。
...
所以，在这一点上，赋值语句本质上是`var = 6 * 7`。现在,`回到Y`返回`四十二`作为结果返回值`it.next( 7 )`呼叫。
...
这是一个“有趣”的例子贯穿在你的脑海里。Did you keep it straight?
...
最引人注目的是违约。`阵列`迭代器_不关心发送的任何消息_下一个（..）`调用，所以值`二`，`三`，和`四`are essentially ignored. 而且，因为`迭代器_没有显式的_返回`值（与以前使用的值不同）`\* foo()`），这`产量\*`表达得到`未定义`当它完成。`委派的例外情况！
