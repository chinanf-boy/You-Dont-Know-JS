...
-   [构造函数]`（obj）`：创建一个新的视图和缓冲，遍历数组或对象`obj`to copy its contents
...
要从一个映射中获取值列表，请使用`值（..）`，它返回一个迭代器。在第2章和第3章中，我们介绍了顺序处理迭代器（如数组）的各种方法，如`…`扩展运算符和`对..`环。此外，第6章中的“数组”包括`数组（…）`method in detail. 考虑：
