...
现在,这会看起来很奇怪,它不太可能去匹配其他6文献给出的建议ㄢ但我有理由疯狂ㄢ
...
在这里,我们强迫创建一个新的`J`对于每个迭代,然后闭包都以相同的方式工作ㄢ我更喜欢前一种方法,那就是我特别赞同的能力ㄢ`为了(让ⅆ)..`形式ㄢ可以说它有点多ㄢ_隐性的_但它是_明确的_够了,够实用了,适合我的口味ㄢ
...
从6,函数的声明出现在块现在规定范围的那块ㄢ前6,规范没有要求,但还是做了很多的实现ㄢ所以现在规范符合现实ㄢ
...
但你怎么能省略第一个呢?`X`没有能力传递任何价值的争论(甚至不是)`未定义`"我漏掉了这个论点ℽ?
...
正如您所看到的,默认值表达式是经过求值的,这意味着它们只在需要时和需要时运行,也就是说,当参数参数被省略或是`未定义`ㄢ
...
介绍了一种新的句法特征称为6_解构_如果你相反,这可能会稍微不那么混乱ㄢ_结构分配_ㄢ要理解这个意义,请考虑: 
...
提醒一句关于解构: 它可能是诱人的解构的任务列表都在一个单一的线已在我们迄今所做的探讨ㄢ然而,传播解构分配模式多行的这是一个更好的主意,使用适当的压痕,就像你会在JSON或文字对象的价值ℴℴ可读性的缘故ㄢ
...
有两`ⅆ`这里使用的操作符,它们都在数组中收集值(`Z`和`W`),虽然`ⅆZ`从第一个数组参数剩下的值中收集,同时`ⅆW`从第一部分剩下的主要论点中收集起来ㄢ
...
不如虚伪的诺言好`对象(ⅆ)`(只是它很浅),但我认为它比手动方法更好ㄢ尽管如此,它仍然是冗长而重复的ㄢ
...
如果总是觉得多余的话`X: X`到处都是好消息ㄢ如果需要定义与词汇标识符相同的属性,则可以将其从`X: X`到`X`ㄢ考虑: 
...
任何有效表达式都可以出现在`㄰..]`位于对象文字定义的属性名称位置ㄢ
...
**注: **我们将讨论`对象`再次在第6章ㄢℽ`对象ㄢsetprototypeof(..)`静态函数"提供额外的细节`对象ㄢsetprototypeof(..)`ㄢ也看到了ℽ`对象(ⅆ)`"静态函数ℽ的另一种形式涉及`O2`典型的以`O1`ㄢ
...
这里,内心`"{谁}ℽ`当合并字符串时,插入字符串文字对我们来说有点方便ㄢ`谁`可变的`"Sℽ`字符串,而不是`谁+"Sℽ`ㄢ有些情况下,嵌套插入字符串的文字是有帮助的,但如果你发现自己经常做这种事情,或者发现自己嵌套了好几个层次,就要小心ㄢ
...
箭头函数定义由一个参数列表(零个或多个参数)及其周围组成ㄢ`(..)`如果没有一个参数),后面是`= >`标记,后跟一个函数体ㄢ
...
在这个例子中,我做的唯一修改是删除`功能`,`返回`,和一些`{ ..}`然后插入`= >`和一个`VaR`ㄢ这是对代码可读性的显著改进吗?无聊的.
...
正如你所看到的,`对..`在键/索引中循环`一`阵列,而`对..`在值中循环`一`ㄢ
...
`对..`循环可以过早地停止,就像其他循环一样ㄢ`打破`,`持续`,`返回`(如果在函数中),抛出异常ㄢ在任何这些情况下,迭代器的`return(..)`如果需要,函数将自动调用(如果存在),让迭代器执行清理任务ㄢ
...
-   `测试(ⅆ)`不注意`lastIndex`的值,并且总是从输入字符串的开始执行匹配ㄢ
-   因为我们的模式没有`^`输入锚的开始,搜索`"fooℽ`可以自由前进通过整个字符串寻找一个匹配ㄢ
-   `lastIndex`不更新`测试(ⅆ)`ㄢ
...
-   `测试(ⅆ)`使用`lastIndex`作为准确和唯一的位置`STR`找火柴ㄢ前面没有人在找比赛,要么就在那儿ㄢ`lastIndex`地位与否ㄢ
-   如果比赛成功,`测试(ⅆ)`更新`lastIndex`在比赛后立即指出角色ㄢ如果比赛失败,`测试(ⅆ)`重置`lastIndex`回到`零`ㄢ

正常的非粘性模式,否则`^`-根到输入的开始是自由前进在输入字符串寻找一个匹配ㄢ但是粘滞模式限制了模式匹配的位置ㄢ`lastIndex`ㄢ

当我`Y`在模式的开始部分是一个虚拟锚点,它是相对的(也就是说限制了比赛的开始)ㄢ`lastIndex`位置ㄢ

**警告: **在以前的关于这个主题的文献中,有人断言这种行为是相似的ㄢ`Y`这意味着一个`^`(输入开始)模式中的锚点ㄢ这是不准确的ㄢ稍后我们将在"锚粘ℽ中进一步解释ㄢ

#### 粘粘的定位

似乎很奇怪地限制了它的使用ㄢ`Y`对于重复的匹配,您必须手动确保`lastIndex`处于正确的位置,因为它没有提前匹配的能力ㄢ

这里有一个可能的场景: 如果你知道你关心的比赛总是在一个数字的倍数处,`零`,`十`,`二十`您可以只构建一个匹配您关心的有限模式,然后手动设置ㄢ`lastIndex`每次比赛前,这些固定位置ㄢ
...
但是,如果您正在解析一个字符串,而不是在固定位置那样格式化,那么找出要设置的位置ㄢ`lastIndex`在每场比赛之前都可能是站不住脚的ㄢ

这里有一个节省的细微差别需要考虑ㄢ`Y`要求`lastIndex`处于比赛的准确位置ㄢ但这并不是严格要求的ㄢ_你_手动设置`lastIndex`ㄢ

相反,你可以用这样一种方式来构建你的表达方式,在每一个主要的匹配之前,在你关心的事情之前和之后,在你关心的下一件事情之前,把它们捕获ㄢ

因为`lastIndex`在匹配结束后,如果匹配了所有的字符,将设置为下一个字符,`lastIndex`将始终处于正确的位置`Y`从下一次开始的模式ㄢ

**警告: **如果不能以这样一种充分模式化的方式来预测输入字符串的结构,这种技术可能并不合适,您可能无法使用ㄢ`Y`ㄢ

结构化字符串输入可能是最实际的场景ㄢ`Y`将能够在字符串中执行重复匹配ㄢ考虑: 
...
之所以如此,是因为我提前知道了输入字符串的结构: 总是有一个数字前缀ㄢ`"1ㄢℽ`在想要的比赛之前(`"fooℽ`,或者后面的空格,或者字符串的结尾(`美元`锚)ㄢ因此,我构建的正则表达式在每个主匹配中捕获所有这些,然后使用匹配组ㄢ`()`为了方便起见,我真正关心的东西被分开了ㄢ

第一场比赛之后(`"1ㄢFooℽ`),这`lastIndex`是`七`,这已经是开始下一场比赛所需要的位置了ㄢ`"2ㄢ酒吧ℽ`等等ㄢ

如果你打算使用`Y`重复比赛的粘性模式,你可能会想寻找机会ㄢ`lastIndex`自动定位,正如我们刚才演示ㄢ

#### 粘性与全球

有些读者可能意识到你可以效仿这样的东西ㄢ`lastIndex`-相对匹配`G`全局匹配标志和`行政长官(ⅆ)`方法,因此: 
...
虽然那是真的`G`模式匹配`行政长官(ⅆ)`开始匹配`lastIndex`的当前值,并更新`lastIndex`每一场比赛(或失败)之后,这不是同一回事ㄢ`Y`行为ㄢ

注意在前一段代码中`"书ℽ`位于位置`六`被匹配和发现的第二个`行政长官(ⅆ)`即使当时打电话,`lastIndex`是`四`(上一场比赛结束时)ㄢ为什么?因为正如我们前面所说的,非粘性匹配可以在匹配中向前移动ㄢ一个粘滞模式表达式在这里会失败,因为它不允许向前移动ㄢ

除了在匹配行为方面不受欢迎之外,另一个缺点就是使用ㄢ`G`而不是`Y`是的,`G`更改某些匹配方法的行为,例如`str.match(RE)`ㄢ
...
看一次所有的比赛是怎么回来的?有时候没关系,但有时候这不是你想要的ㄢ

这个`Y`粘贴标志会给你时间与事业一样进行匹配`测试(ⅆ)`和`比赛(ⅆ)`ㄢ只要确定`lastIndex`每场比赛总是处于正确的位置!

#### 锚粘

正如我们早些时候警告过的那样,认为粘滞模式意味着一个模式开始时是不准确的ㄢ`^`ㄢ这个`^`锚在正则表达式中有明确的含义,它是_没有改变_粘性模式ㄢ`^`是一个锚_总是_指输入的开始,以及_是不是_以任何方式相对于`lastIndex`ㄢ

除了可怜的/不准确的关于这一主题的文档,混乱是进一步加强由于旧pre-es6实验与Firefox的粘滞模式_做_制作`^`相对于`lastIndex`所以这种行为已经存在多年了ㄢ

6选择不这样做ㄢ`^`在模式中意味着绝对和唯一的输入开始ㄢ

因此,像这样的模式`和`将永远只找到一个`"fooℽ`字符串开头的匹配,_如果允许在那里匹配_ㄢ如果`lastIndex`是`零`比赛将失败ㄢ考虑: 
...
底线: `Y`加`^`加`lastIndex > 0`是一个不兼容的组合,总是会导致一个失败的匹配ㄢ

**注: **而`Y`不改变意思`^`无论如何,`M`多行模式_做_,这样`^`表示输入开始_或_正文开始后换行ㄢ所以,如果你结合`Y`和`M`旗帜一起为一个模式,你可以找到多个`^`根字符串中的根匹配ㄢ但是请记住: 因为它是`Y`粘糊糊的,你得弄清楚`lastIndex`每一个后续时间都指向正确的新行位置(可能是匹配到行的结尾),否则将不会进行后续匹配ㄢ
...
这是一个小的差别,但6规范要求表达的标志是这个顺序列出: `"gimuyℽ`不管原始模式是用什么顺序指定的ㄢ这就是两者之间差异的原因ㄢ`/搞笑`和`"GIℽ`ㄢ
...
此前的6,`3`建设将抛出一个错误,但作为6可以覆盖国旗在复制ㄢ
...
**注: **虽然你是指定在不同基地的数量,数量的数学值被存储,并默认输出解释永远是10ㄢ前一段代码中的三个变量都有`四十二`存储在它们中的值ㄢ
...
ES5继续允许浏览器扩展的八进制形式(包括这种不一致),但在严格的模式ㄡ八进制字面(`052`)的形式是不允许的ㄢ这限制了主要是因为很多开发商已经习惯(从其他语言)看似无害化前缀另有10号`零`用于代码对齐的目的,然后遇到一个意外的事实,即他们完全改变了数值!

6继续变化/变化如何数文字以外的普通数字可以表示的遗产ㄢ现在有正式八进制格式ㄡ经过修改的十六进制形式和全新的二进制形式ㄢ出于Web兼容性的原因,旧八进制`052`在非严格模式下,表单仍将是合法的(尽管未指明),但真的不应该再使用了ㄢ
...
只有小数形式可以是10ㄢ八进制ㄡ十六进制和二进制都是整数形式ㄢ
...
事实上,您可以在任何基础上以这样的方式表示一个数字ㄢ`二`到`三十六`尽管你很少去标准基地: 2, 8, 10和16ㄢ

## Unicode

我想说的是,这一部分并不是你想知道的关于Unicode资源的全部内容ㄢ我想介绍一下你需要知道的东西ㄢ_改变_Unicode在6,但我们不会做得比这更深ㄢMathias Bynens(<http://twitter.com/mathias>)关于JS和Unicode的广泛和精辟的书面和口头发言(参见<https://mathiasbynens.be/notes/javascript-unicode>和<http://fluentconf.com/javascript-html-2015/public/content/2015/02/18-javascript-loves-unicode>)ㄢ

Unicode字符,范围从`0x0000`到`0xffff`包含你可能见过或与之交互的所有标准印刷字符(用各种语言)ㄢ这组字符被称为_基本多语言平面(BMP)_ㄢBMP甚至包含有趣的符号,这样酷的雪人: ☃(U + 2603)ㄢ

除了BMP集之外,还有许多其他扩展Unicode字符,它的范围包括`0x10ffff`ㄢ这些符号常被称为_星光_符号,这是给16的集合的名称ㄢ_飞机_BMP以外的字符(例如层/分组)ㄢ星符号的例子包括(U + 1d11e)和(U + 1f4a9)ㄢ
...
然而,这`\uxxxx`Unicode转义仅支持四个十六进制字符,因此只能用这种方式表示BMP字符集ㄢ代表使用Unicode逃跑之前6星体而言,你需要使用一个_代理对_基本上是两个规格
...
正如你所看到的,不同之处在于`{ }`在转义序列中,该序列允许包含任意数量的十六进制字符ㄢ因为你只需要六在Unicode代表最高可能的代码点的值(即,0x10ffff),这是足够的ㄢ

### Unicode感知字符串操作

默认情况下,JavaScript字符串操作和方法对字符串值中的星形符号不敏感ㄢ因此,他们分别对待每一个BMP字符,甚至是构成另一个星体角色的两个代理半角色ㄢ考虑: 
...
回忆起"`对..`环ℽ部分在本章前面,6字符串有内置的迭代器ㄢ这个迭代器恰好是Unicode感知的,这意味着它将自动输出一个星体符号作为一个值ㄢ我们利用了`ⅆ`在数组文字中展开运算符,它创建字符串符号的数组ㄢ然后我们只检查这个结果数组的长度ㄢ6的`Array.from(..)`基本上是一样的事情`[ⅆⅆXYZ ]`但我们将在第6章中详细介绍该实用程序ㄢ

**警告: **应该注意的是,构建和耗尽一个迭代器以获得字符串的长度在性能上是相当昂贵的,相对而言,与理论上优化的本地实用程序/属性所做的工作相比ㄢ

不幸的是,完整的答案并不是那么简单明了ㄢ除了代理对(字符串迭代器所关心的)之外,还有一些特殊的Unicode代码点,它们以其他特殊的方式运行,这很难解释ㄢ例如,有一组代码点修改前一个相邻字符,称为_组合用附加符号_ㄢ
...
基本上,`规范化(ⅆ)`像一个序列`E u0301ℽ`和规范它`"\xE9ℽ`ㄢ如果结合了合适的Unicode字符,标准化甚至可以合并多个相邻的组合标记: 
...
不幸的是,这里的标准化并不完全完美ㄢ如果有多个组合标记修改单个字符,则可能无法得到预期的长度计数,因为可能没有一个定义的标准化字符表示所有标记的组合ㄢ例如:
...
你沿着这个兔子洞走得越远,你就越意识到很难得到一个"长度ℽ的精确定义ㄢ_石墨烯_ℴℴ并不总是严格地与程序处理意义上的单个"字符ℽ相关ㄢ

**提示: **如果你想看到的是如何深入这个兔子洞,检查"字形簇边界ℽ算法([HTTP: / / www.unicodeㄢorg /报告/ tr29 / # grapheme_cluster_boundaries](http://www.Unicode.org/reports/tr29/#Grapheme_Cluster_Boundaries))ㄢ

### 角色定位

类似长度并发症,它实际上意味着问,"什么是字符在位置2?ℽ天真的pre-es6回答来自`charAt(..)`,不尊重一个星体性质的原子性,也考虑到结合标志ㄢ
...
所以,是6给我们一个Unicode版本`charAt(..)`?不幸的是,我写这篇文章的时候,有这样一个工具,是考虑post-es6在提案ㄢ
...
**警告: **提醒前面的警告: 每当您想获得单个字符时,构造和耗尽迭代器是ⅆ不是很理想,性能聪明ㄢ让我们希望我们有一个内置的优化工具很快,post-es6ㄢ
...
另一个方向呢?Unicode感知版本`字符串ㄢfromcharcode(..)`是6的`字符串ㄢfromcodepoint(..)`: 
...
等等,我们可以合并吗?`字符串ㄢfromcodepoint(..)`和`codepointat(..)`获得一个更好的Unicode感知版本`charAt(..)`从早期的?是的!
...
还有很多其他的字符串方法我们没有在这里讨论,包括`touppercase()`,`tolowercase()`,`子串(..)`,`IndexOf(..)`,`切片(..)`还有十几个ㄢ所有这些都没有更改或增强为完整的Unicode意识,所以您应该非常小心-可能只是避免它们!ℴℴ当使用包含星体符号的字符串时ㄢ

还有几个字符串方法`替换(ⅆ)`和`比赛(ⅆ)`ㄢ值得庆幸的是,6带来Unicode意识的正则表达式,得到"UNICODEℽ在本章的前面ㄢ

好的,我们有!JavaScript的Unicode字符串的支持明显优于pre-es6(尽管还不完善)与各种添加我们刚刚覆盖ㄢ

### Unicode标识符名称

Unicode也可以用于标识符名称(变量ㄡ属性等)ㄢ6之前,你可以做的与Unicode转义,如: 
...
有一套复杂的规则围绕着哪些Unicode字符被允许ㄢ此外,只有当它们不是标识符名称的第一个字符时才允许ㄢ

**注: **Mathias Bynens有一个伟大的职位(<https://mathiasbynens.be/notes/javascript-identifiers-es6>)所有的细节ㄢ

在标识符名称中使用这种不寻常字符的原因是相当罕见的和学术性的ㄢ通过编写依赖于这些深奥功能的代码,您通常不会得到最好的服务ㄢ

## 符号

与6,在相当长的一段时间是第一次,一个新的原始类型已被添加到JavaScript的`符号`ㄢ然而,与其他原始类型不同的是,符号没有文字形式ㄢ
...
-   你不能也不应该使用`新的`具有`符号(..)`ㄢ它不是构造函数,也不是生成对象ㄢ
-   传递给`符号(..)`是可选的ㄢ如果传递的话,它应该是一个字符串,可以对符号的用途进行友好的描述ㄢ
-   这个`类型`输出是一个新值(`"符号ℽ`这是识别符号的主要方法ㄢ
...
类似于原始字符串值不是实例的`字符串`符号也不是`符号`ㄢ如果出于某种原因,您想要构造一个符号值的包装器对象形式,您可以执行以下操作: 
...
**注: ** `symobj`在这个片段中可以互换`对称`任何一种形式都可以在所有地方使用符号ㄢ没有太多的理由使用盒装包装器对象表单(`symobj`而不是原始形式ㄢ`对称`)ㄢ保持对其他原语的类似建议,最好还是选择`对称`结束`symobj`ㄢ

符号本身的内部值,称为符号值ㄢ`名称`ℴℴ隐藏在代码中,无法获得ㄢ可以将这个符号值视为自动生成的ㄡ唯一的(在应用程序中)字符串值ㄢ

但是,如果值是隐藏的,无法获得,在所有具有象征的意义是什么?

符号的要点是创建一个字符串值,它不能与任何其他值冲突ㄢ因此,例如,考虑使用符号作为表示事件名称的常量: 

```js
const EVT_LOGIN = Symbol( "event.login" );
```

然后你会使用`evt_login`代替一般字符串文字`"事件ℽ登录`: 
...
这里的好处是`evt_login`保存一个不能被任何其他值复制(意外或其他)的值,因此不可能有任何混乱的事件被分派或处理ㄢ

**注: **封面下面`evthub`在前一段代码中假定的实用程序几乎可以肯定地使用`evt_login`直接作为跟踪事件处理程序的内部对象(hash)中的属性/键的参数ㄢ如果`evthub`相反,需要将符号值用作真正的字符串,它需要显式强制使用`字符串(..)`或`tostring()`因为不允许符号的隐式字符串强制ㄢ

您可以在对象中直接使用一个符号作为属性名/键,例如您希望处理为隐藏或使用中的特殊属性ㄢ重要的是要知道,虽然你打算这样对待它,但它不是ㄢ_事实上_隐蔽的或不可触及的财产ㄢ
...
这个`实例`这里的符号值是一个特殊的ㄡ几乎是隐藏的ㄡ类似于元的属性,静态地存储在`happyface()`函数对象ㄢ

它也可以是一个普通的老财产ㄢ`__instance`而且行为会是一样的ㄢ一个符号的使用只是提高了元编程风格,保持这`实例`与其他正常属性分开的属性ㄢ

### 符号注册表

在最后几个例子中使用符号的一个轻微的缺点是`evt_login`和`实例`变量必须存储在外部范围(甚至是全局范围),或者以某种方式存储在公开可用的位置,以便需要使用符号的所有代码部分都可以访问它们ㄢ

为了帮助组织代码以访问这些符号,您可以使用_全局符号注册表_ㄢ例如:
...
`符号(ⅆ)`在全局符号注册表中查看是否已将符号存储在所提供的描述文本中,如果返回,则返回符号ㄢ如果没有,它创造

但这也意味着应用程序的任何部分都可以从注册表中检索符号ㄢ`符号(ⅆ)`只要使用匹配描述名即可ㄢ

讽刺的是,符号基本上是用来取代_魔力字符串_(在应用程序中给定特殊含义的任意字符串值)ㄢ但你准确地使用_魔术_描述字符串值以在全局符号注册表中唯一地标识/定位它们!

为了避免意外碰撞,您可能希望使您的符号描述非常独特ㄢ这样做的一个简单的方法是在其中包括前缀/背景/命名空间信息ㄢ
...
我们使用魔法字符串值ㄢ`"extractvaluesㄢ解析ℽ`因为注册表中的任何其他符号都不可能与该描述相冲突ㄢ
...
除了符号注册表提供的帮助,全局存储这些值,我们在这里看到的一切都可以通过实际使用魔术字符串来完成ㄢ`"extractvaluesㄢ解析ℽ`作为关键,而不是符号ㄢ改进存在的元编程水平超过功能水平ㄢ

您可能有机会使用存储在注册表中的符号值来查找存储在其下面的描述文本(key)ㄢ例如,您可能需要向应用程序的另一个部分发出信号,如何在注册表中定位符号,因为无法传递符号值本身ㄢ

您可以使用一个已注册符号的描述文本(key)检索`的象征ㄢ关键(..)`: 
...
这清楚地表明属性符号实际上并不是隐藏的或不可访问的,因为您可以在`对象ㄢgetownpropertysymbols(..)`列表ㄢ

#### 内置的符号

6有许多预定义的内置符号,揭露各种元行为对JavaScript对象的值ㄢ然而,这些符号是_不_注册在全局符号注册表中,正如人们所期望的那样ㄢ

相反,它们作为属性存储在`符号`函数对象ㄢ例如,在"`对..`在本章的前面部分,我们介绍了`symbol.iterator`价值: 
...
规范使用`@ @`前缀符号是指内置符号,最常见的符号是: `@ @迭代器`,`@ @ tostringtag`,`@ @ toprimitive`ㄢ还有好几种定义,尽管它们可能不会经常使用ㄢ

**注: **请参见第7章中的"众所周知的符号ℽ,以获得关于这些内置符号如何用于元编程目的的详细信息ㄢ

## 回顾

6加一堆新的语法形式的JavaScript,所以有很多学习!

其中大部分旨在减轻常见编程习惯用法的疼痛点,例如将默认值设置为函数参数,并将参数的"RESTℽ收集到数组中ㄢ解构是简洁地表达值的数组分配一个强大的工具和嵌套的对象ㄢ

而特征`= >`箭头函数看起来也都是短而漂亮的语法,它们实际上有非常特殊的行为,只有在适当的情况下才应该故意使用ㄢ

扩展Unicode支持,正则表达式的新技巧,甚至一个新的原语`符号`型圆了6句法演变ㄢ
