...
如果你进一步研究，你会发现你只能省略参数对端（即右边）通过简单的传递参数比预期的少，但你不能省略的参数在中间或在参数列表的开始。It's just not possible.
...
让我们深入探究`x，x，…}`从上一段代码中提取的语法。如果匹配的属性名称与您要声明的变量相同，则可以实际缩短语法：
...
超级的`在借鉴`foo() O2。`方法是静态锁定的。`O2`，特别是`\[原型]`属于`O2`。`超级的`这里基本上是`getprototypeof（O2）的对象。`——解决`O1`当然——这是它如何找到和调用的。`foo() O1。`。`详情请看
...
如果一个`数`value is encountered in the`价值观`数组，我们放`“$”`在它前面，把它格式化为小数点后两位数。`（2）固定`。否则，我们的价值就不会受到影响。
...
我真的认为缺乏`返回`和外`{ ..}`部分掩盖了这样一个事实：`减少（…）`调用是唯一的语句`dollabillsyall（..）`函数，其结果是调用的预期结果。此外，训练有素的眼睛是如此习惯于寻找单词。`功能`在寻找范围界限的代码中，现在需要查找`= >`标记，这在代码中很难找到。
...
**警告：**默认情况下，普通对象不适合`对..`循环。那`对..`在任何对象上循环以获得一组我们定义的值。
