...
重要的是要注意第一个例外（或拒绝的承诺）。`满足（…）`将_不_结果在第一`拒绝（…）`被调用，因为该处理程序只对第一个原始PR的解析作出响应。`然后（…）`被拒绝，受到拒绝。
...
`Promise.resolve(..)`创建一个解析为传递的值的承诺。让我们来比较它如何工作，以更手动的方法：
...
决心（…）`和`Promise.resolve(..)`可以接受承诺并通过其状态/决议，`拒绝（…）`和`承诺，拒绝（…）`不要区分它们收到的值。所以，如果你拒绝承诺或thenable，诺言thenable本身将被设置为拒绝的理由，而不是它的潜在价值。`承诺（全部）]）
