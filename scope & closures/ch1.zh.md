...
3.  **代码生成: **接受AST并将其转换为可执行代码的过程. 这部分根据语言ㄡ目标平台等不同而有很大差异. 
...
合理的假设是_编译器_将生成可以由这个伪代码总结的代码: "为一个变量分配内存,并标记它`一`然后坚持这个价值观`二`进入那个变量. "不幸的是,这不太准确. ". 
...
稍微圆滑的那一刻,你也可以认为"右"而不是"他/她是检索源(值)",这意味着该手段"去获得价值ⅆ". 
...
参考`一`是一个RHS的参考,因为没有被分配到`一`在这里.相反,我们正在寻找检索值. `一`以便将值传递给`控制台(ⅆ)`. 
...
你可能错过了暗示. `= 2`在这个代码片段中. 当价值发生时`二`作为参数传递给`富(ⅆ)`函数,在这种情况下`二`值**分配**对参数`一`. (隐式)赋值给参数`一`,执行该查询. 

还有一个RHS的参考价值`一`并将结果值传递给`控制台(ⅆ)`. `控制台(ⅆ)`需要一个引用来执行. 这是一个右查`慰问`对象,然后发生一个属性解析,看看它是否有一个方法被调用. `日志`. 

最后,我们也可以想象,有一个左/右通过价值交换`二`(通过变量的方式)`一`查到的RHS)`记录(ⅆ)`. 本机内部实现`记录(ⅆ)`我们可以假设它有参数,第一个参数(也许是调用)`arg1`)有一个LHS参考查找,然后分配`二`它. 

**注: **您可能会试图对函数声明进行概念化. `函数(a){ⅆ`作为一个普通变量声明和赋值,例如`变量Foo`和`函数(a){ⅆ`. 这样做,你可能会认为这个函数声明为涉及该查. 
