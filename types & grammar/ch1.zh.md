...
呸！我们将使用这个粗略的定义（与规范的措辞相同的定义）：a_类型_是一个固有的、内置的特性集，它唯一地标识一个特定值的行为，并将其与其他值区别开来，这两个属性都是引擎。**并向开发人员**。
...
这个_价值_四十二`有一种内在的类型`数`，和其`类型_不能改变。另一个值，比如_“42”`与`一串`类型，可以创建`从_这个_数`价值`四十二`通过一个被调用的过程`强制**（见第4章）。**如果你使用
