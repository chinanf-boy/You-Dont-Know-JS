...
既然有内部定义`[得到]`从属性获取值的操作应该很明显，也有默认值。`[[Put]]`操作。

可能认为，对对象上的属性赋值只会调用`[[Put]]`在问题对象上设置或创建该属性。但情况比那更微妙。

当调用`[[Put]]`，如何表现不同，基于多项因素，包括（最impactfully）属性是否已在对象上或不。
...
如果该属性尚未存在于所讨论的对象上，则`[[Put]]`操作更加细致入微和复杂。在讨论第5章时，我们将重新讨论这个场景。`[原型]`让它更清晰。

### 干将与二传手

默认的`[…]`和`[得到]`对象的操作完全控制值a

**注：**使用该语言的未来/高级功能，可以重写默认值。`[得到]`或`[[Put]]`整个对象的操作（不仅仅是每个属性）。这超出了我们在本书中讨论的范围，但稍后将在“不知道JS”系列中讨论。
...
对象是键/值对的集合。这些值可以作为属性访问，通过`propname。`或`[“propname”]`语法。每当访问一个属性时，引擎实际上调用内部默认值。`[得到]`（和操作`[[Put]]`用于设置值），它不仅直接在对象上查找属性，而且将遍历对象。`[原型]`链（见第5章）如果找不到。
