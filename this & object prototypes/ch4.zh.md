...
drive()`，和“B”和“C”重写（多态），方法，当`D`引用`drive()`它应该使用哪种版本？`B：drive()`或`C：drive()`）？`这些并发症比这一眼更为深刻。我们只在这里讨论它们，因此我们可以对比JavaScript机制的工作方式。

<img src="fig2.png">

JavaScript更简单：它不提供“多继承”的本地机制。很多人看到这是一件好事，因为节省的复杂性。
...
由于复制操作，`小型车`将分开操作一些`车辆`。如果将属性添加到`小型车`, it will not affect`车辆`反之亦然。
